...
//!
//! Ports the minimal machinery needed to dispatch DRPC RPCs over the
...
//!
//! Live RPCs so far: `CommitQuota`, `UpdateJob`. Unknown method paths
//! return [`RpcError::Unimplemented`] so Go clients can transparently
//! recognise missing RPCs via `drpcerr.Unimplemented` (code 12).

use std::sync::Arc;

use coder_agent_rpc::RpcError;
use coder_core::provisioner::{LogLevel, LogSource};
use coder_core::{InsertProvisionerJobLogsInput, ProvisionerStore};
