...
//! `TEST_DATABASE_URL` environment variable to the connection string,
//! e.g. `postgres://postgres:postgres@127.0.0.1:5432/coder_test`.
//!
//! When `TEST_DATABASE_URL` is **not** set every test is silently skipped
//! so that `cargo test --workspace` never fails in environments without
//! a database.
...
fn authed_request(method: Method, uri: &str, token: &str) -> Result<Request<Body>, http::Error> {
    Request::builder()
        .method(method)
        .uri(uri)
...
async fn login_user(app: &Router, email: &str, password: &str) -> Result<String, Box<dyn Error>> {
    let resp = call(
        app.clone(),
        json_request(
            Method::POST,
