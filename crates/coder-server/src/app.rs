        ...
        .nest(
            ...
            Router::new()
                ...
                .route(
                    ...
                    get(get_previous_template_version_by_org_and_name),
                )
                .route(
...
async fn get_previous_template_version_by_org_and_name(
    State(state): State<AppState>,
    Path((organization, template_name, version_name)): Path<(String, String, String)>,
    headers: HeaderMap,
) -> Result<Response, AppError> {
    let Some(_context) = authenticate_request(&state, &headers).await? else {
        return Ok(unauthorized_response("Missing or invalid session token."));
    ...
    let org = match org {
        Some(o) => o,
        None => return Ok(not_found_response("Organization not found.")),
    };

    // Find the template by name within the organization.
    let templates = state
        .store
        .list_templates(TemplateListFilter {
    ...
    let prev = state
        .store
        .get_previous_template_version(org.id, &version_name, Some(template.id))
        .await?;

    match prev {
        Some(v) => {
            let job = state.store.find_provisioner_job(v.job_id).await?;
            match job {
                Some(j) => {
                    let response = template_version_response(&v, &j);
                    Ok((StatusCode::OK, Json(response)).into_response())
                }
                None => Ok(not_found_response("No previous template version found.")),
            }
        }
        None => Ok(not_found_response("No previous template version found.")),
    }
