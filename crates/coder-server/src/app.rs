    ...
    for region in &state.config.derp_regions {
        let nodes: Vec<DERPNode> = region
            ...
            .map(|node| DERPNode {
                ...
                ipv4: None,
                ipv6: None,
                stun_port: node.url.port().map_or(3478, i32::from),
                stun_only: false,
                derp_port: node.url.port().map_or(443, i32::from),
...
mod tests {
    ...
    use coder_core::{
        ...
        WorkspaceAgentStatInput, WorkspaceBuildRecord, WorkspaceBuildStatsInput,
        WorkspaceConnectionLatencyMs, WorkspaceDeploymentStatsResponse, WorkspaceProxyHealthInput,
        WorkspaceProxyHealthRecord, WorkspaceResourceRecord, WorkspaceStatsWorkspaceInput,
    };
    use serde::Serialize;
    use serde_json::{Value, json};
    use time::OffsetDateTime;
    ...
    struct FakeStore {
        ...
        workspace_resources: Mutex<HashMap<Uuid, WorkspaceResourceRecord>>,
