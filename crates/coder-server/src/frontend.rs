...
///
/// The `folder` path is resolved relative to this crate's `Cargo.toml`, so
/// we point up two levels (`crates/coder-server` → workspace root) to the
/// conventional `site/out/` location.
#[derive(Embed)]
#[folder = "../../site/out"]
struct Asset;
...
/// Builds the concrete HTTP response for an embedded asset.
///
/// `bytes` is a `Cow` so `rust-embed`'s compile-time static slices skip the
/// allocation entirely. Response construction uses only static header
/// values, so the builder cannot fail at runtime.
fn build_response(
    status: StatusCode,
    bytes: Cow<'static, [u8]>,
...
pub(crate) async fn router_fallback_handler(uri: Uri) -> Response {
    let path = uri.path();
...
mod tests {
    ...
    async fn unknown_path_falls_back_to_index_html() {
        ...
        let response = router_fallback_handler(uri).await;
