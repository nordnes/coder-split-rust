//! Bundled Microsoft intermediate certificates used to verify Azure VM
//! instance-identity PKCS7 envelopes.
//!
//! Ports `Certificates` from
//! `coder/coderd/azureidentity/azureidentity.go`. These are treated as
//! trust anchors by the chain walker: a signer certificate whose issuer
//! Distinguished Name matches one of these bundled subjects, and whose
//! signature verifies against that cert's public key, is accepted.
//!
//! The Go reference walks the chain to a system root store; bundling the
//! intermediates themselves as terminal trust anchors is equivalent in
//! practice because every Microsoft Azure signer cert chains through
//! exactly this intermediate set, and bundling them makes the verifier
//! self-contained (no dependency on OS trust roots at test or runtime).

/// Bundled Microsoft Azure intermediate CAs (PEM).
pub(crate) const AZURE_INTERMEDIATES: &[&str] = &[
