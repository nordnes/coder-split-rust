...
/// `coder/coderd/awsidentity/awsidentity.go` — certificates are copied
/// verbatim from the Go reference for behavioural parity, except for the
/// `Other` commercial-region cert which the Go reference has not yet
/// rotated (its bundled copy expired 2024-06-05). We substitute the
/// current public RSA cert published at
/// <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/regions-certs.html>
/// (validity 2024-04-29 → 2029-04-28), so signatures generated by AWS EC2
/// in commercial regions continue to verify.
///
/// Every bundled cert has its `notBefore` / `notAfter` annotated inline so
/// future drift is obvious in `git blame`.
///
/// Additional partition-specific regions not covered here (e.g. `us-iso*`,
/// `us-isob*`, and any newer `cn-*` partitions beyond the one Go already
/// bundles) are NOT shipped as defaults. Operators on those partitions
/// must inject their own certs through [`AwsInstanceVerifier::with_certificates`]
/// — or, for deployment-wide configuration, by setting the
/// `CODER_AWS_INSTANCE_IDENTITY_CERTS_DIR` environment variable / the
/// `--aws-instance-identity-certs-dir` CLI flag, which loads every
/// `*.pem` / `*.crt` file in the directory at startup and appends them to
/// the bundled set.
pub(crate) const DEFAULT_CERTIFICATES: &[&str] = &[
    // "Other" — commercial regions except those listed below.
    // notBefore: 2024-04-29, notAfter: 2029-04-28.
    // Source: https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/regions-certs.html
    // (The Go reference still carries the 2014 → 2024 cert that expired
    // 2024-06-05; we refresh to the current public RSA cert here.)
    "-----BEGIN CERTIFICATE-----\n\
